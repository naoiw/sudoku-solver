...
数独の盤面を入力し、Solve ボタンで解を表示する Web アプリです。入力時はリアルタイムで重複チェックを行い、不正なマスは赤表示されます。不正なマスが1つでもある間は Solve ボタンは押せません。スマートフォンではセルをタップすると数値用キーパッド（テンキー）が表示され、数字を入力できます。
